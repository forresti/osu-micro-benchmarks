...
int main (int argc, char *argv[])
{
    ...
    if (0 == rank) {
        switch (po_ret) {
            ...
            case po_help_message:
                usage(all_sync, "osu_acc_latency");
                break;
        }
    }
    ...
    switch (sync_type){
        case LOCK:
            run_acc_with_lock(rank, win_type);
            break;
         case PSCW:
            run_acc_with_pscw(rank, win_type);
            break;
        case FENCE: 
            run_acc_with_fence(rank, win_type);
            break;
#if MPI_VERSION >= 3
        case LOCK_ALL:
            run_acc_with_lock_all(rank, win_type);
            break;
        case FLUSH_LOCAL:
            run_acc_with_flush_local(rank, win_type);
            break;
        default: 
            run_acc_with_flush(rank, win_type);
            break;
#endif
    }
...
#if MPI_VERSION >= 3
/*Run ACC with flush */
void run_acc_with_flush (int rank, WINDOW type)
{
    int size, i;
    MPI_Aint disp = 0;
    MPI_Win     win;
...
void run_acc_with_flush_local (int rank, WINDOW type)
{
    int size, i;
    MPI_Aint disp = 0;
    MPI_Win     win;
...
void run_acc_with_lock_all (int rank, WINDOW type)
{
    int size, i;
    MPI_Aint disp = 0;
    MPI_Win     win;
